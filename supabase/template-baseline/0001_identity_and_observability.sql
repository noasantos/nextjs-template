...
-- =============================================================================
-- TEMPLATE REFERENCE — not run by supabase db reset. See supabase/template-baseline/README.md
-- and docs/guides/template-baseline-schema.md before applying to a greenfield DB.
-- =============================================================================
--
-- -----------------------------------------------------------------------------
-- Supabase Auth (auth.users) vs public tables — single source of truth
-- -----------------------------------------------------------------------------
-- auth.users: id, email, phone, raw_user_meta_data, … — do NOT duplicate email or
-- phone on public business tables; read via Auth APIs or auth.users (privileged).
-- public.profiles: user_id FK, display fields (full_name, avatar_url), and
-- subscription JSON (billing/plan snapshot). No email/phone columns.
--
-- Subscription → JWT (established flow):
--   1) Source of truth: public.profiles.subscription (jsonb, default {}) — full snapshot
--      for DB/RLS/server actions; may include Stripe ids for webhooks (not all belong in JWT).
--   2) subscription_claims_for_jwt(subscription) projects a WHITELIST into the token
--      (client-readable). Default excludes payment-provider ids and secrets—adjust the function.
--   3) get_user_access_payload_core reads profiles.subscription; custom_access_token_hook
--      merges subscription_claims_for_jwt(...) into claims.app_metadata on every issue/refresh.
--   4) Hook is READ-ONLY: mutable state is updated only via sync_* RPCs / webhooks (service_role).
--      Hook runs on every token issue and refresh—keep get_user_access_payload_core O(1): PK on
--      profiles.user_id + index on user_roles(user_id); avoid unindexed joins here.
--   5) Clients compare access_version (app_metadata) to detect stale JWT; bump when roles or
--      subscription change (sync_user_roles, sync_profile_subscription).
--   6) Immediate revocation or high-risk gates: still enforce server-side (RLS, Edge, Stripe),
--      not JWT alone—tokens are stale until refresh (TTL-bound).
--   7) When to query DB anyway (not for every page view): sensitive ops (money, regulated data,
--      instant revoke) — always; stale JWT suspicion — compare app_metadata.access_version to
--      profiles.access_version via RPC (e.g. get_my_access_payload) after mutations, then refresh.
--      See docs/guides/template-baseline-schema.md ("When to validate on the server with the database").
--
-- JWT / app_metadata hygiene: do not put card data, secrets, or unnecessary PII in claims.
-- Prefer entitlement fields (status, plan_tier, current_period_end). See subscription_claims_for_jwt.
--
-- Roles: public.user_roles + public.app_roles (catalog).
-- Permissions: optional; see commented DDL and get_user_access_payload_core / hook.
--
-- Custom Access Token Hook (required for JWT claims below):
--   Merges into event.claims (preserve Auth-issued iss, aud, exp, sub, role, session_id, etc.—
--   return jsonb_build_object('claims', claims) only; do not drop required claims unless docs allow).
--   Point Supabase Auth at this DB function, e.g. in config.toml:
--     [auth.hook.custom_access_token]
--     enabled = true
--     uri = "pg-functions://postgres/public/custom_access_token_hook"
--   Deploy: Dashboard → Authentication → Hooks, or same URI in hosted project.
-- -----------------------------------------------------------------------------
--
