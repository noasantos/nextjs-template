...
-- created-at-utc: 2026-04-06T17:21:10Z

SET check_function_bodies = false;

--
-- Name: create_session_occurrence_atomic(uuid, uuid, timestamp with time zone, integer, uuid, uuid, text, integer, text, text, text, uuid, integer, text, text, uuid, timestamp with time zone, timestamp with time zone, jsonb); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.create_session_occurrence_atomic(p_psychologist_id uuid, p_patient_id uuid, p_start_time timestamp with time zone, p_duration_minutes integer DEFAULT 50, p_service_id uuid DEFAULT NULL::uuid, p_location_id uuid DEFAULT NULL::uuid, p_notes text DEFAULT NULL::text, p_custom_price_cents integer DEFAULT NULL::integer, p_status text DEFAULT NULL::text, p_recurrence_rule text DEFAULT NULL::text, p_timezone text DEFAULT 'America/Sao_Paulo'::text, p_session_type_id uuid DEFAULT NULL::uuid, p_session_number integer DEFAULT NULL::integer, p_event_title text DEFAULT NULL::text, p_event_description text DEFAULT NULL::text, p_series_id uuid DEFAULT NULL::uuid, p_original_start_datetime timestamp with time zone DEFAULT NULL::timestamp with time zone, p_original_end_datetime timestamp with time zone DEFAULT NULL::timestamp with time zone, p_metadata jsonb DEFAULT '{}'::jsonb) RETURNS jsonb
...
BEGIN
  ...
  -- FIX: psychologist_clients → psychologist_patients (renamed in 20260217095717)
  SELECT
    id,
    patient_id,
  ...
  IF p_service_id IS NOT NULL THEN
    SELECT
      ...
      rv.label_pt AS catalog_name
    INTO v_service_record
    FROM public.psychologist_services ps
    LEFT JOIN public.reference_values rv ON rv.id = ps.catalog_id
    ...
    IF FOUND THEN
      ...
      FROM public.psychologist_patient_services
      WHERE psychologist_id = p_psychologist_id
        AND psychologist_patient_id = p_patient_id
  ...
  INSERT INTO public.psychologist_clinical_sessions (
    psychologist_id,
  ...
  INSERT INTO public.psychologist_notes (
    psychologist_id,
    patient_id,
    session_id,
    note_type,
