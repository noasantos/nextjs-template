-- Fix create_session_with_calendar charge insert schema mismatch
-- Keep p_service_id as psychologist_services.id for pricing/session linkage,
-- but do not write service_id into psychologist_client_charges (column does not exist).
...
begin
  ...
  if v_effective_price is not null and v_effective_price > 0 then
    -- CALCULATE NOON UTC DATE FOR CHARGE
    -- 1. Convert p_start_time to p_timezone specific date
    -- 2. Concatenate with 12:00:00+00
    -- 3. Cast back to timestamptz
    ...
    insert into public.psychologist_client_charges (
      ...
      payment_status,
      created_by,
      created_at, -- Uses Noon UTC on session day
      updated_at
    ) values (
      p_psychologist_id,
      p_patient_id,
