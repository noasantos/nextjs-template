/**
 * Unit tests for listAuditLogsAction
 *
...
vi.mock(
  "@workspace/supabase-data/modules/audit/infrastructure/repositories/audit-logs-supabase.repository.codegen",
  () => ({
    AuditLogsSupabaseRepository: vi.fn(),
  })
)
...
describe("listAuditLogsAction", () => {
  ...
  describe("Authentication", () => {
    it("should throw Unauthorized when not authenticated", async () => {
      ...
      await expect(listAuditLogsAction({} as any)).rejects.toThrow("Unauthorized")
    })
    ...
    it("should log security.audit on unauthorized access", async () => {
      ...
      await listAuditLogsAction({} as any).catch(swallowExpectedError)
  ...
  describe("Input Validation", () => {
    it("should validate input with Zod schema", async () => {
      ...
      await expect(listAuditLogsAction({ invalid: "data" } as any)).rejects.toThrow()
    })
  })
  ...
  describe("Logging", () => {
    it("should log on success", async () => {
      const { getClaims } = await import("@workspace/supabase-auth/session/get-claims")
      const { checkActionRateLimit } = await import("@workspace/supabase-data/lib/auth/rate-limit")
      const { AuditLogsSupabaseRepository } =
        await import("@workspace/supabase-data/modules/audit/infrastructure/repositories/audit-logs-supabase.repository.codegen")
      const { logServerEvent } = await import("@workspace/logging/server")
      vi.mocked(getClaims).mockResolvedValue({ claims: { sub: "test-user-id" } } as any)
      vi.mocked(checkActionRateLimit).mockResolvedValue(undefined)
      ...
      vi.mocked(AuditLogsSupabaseRepository).mockImplementation(function MockRepository() {
        return {
          findById: vi.fn(),
