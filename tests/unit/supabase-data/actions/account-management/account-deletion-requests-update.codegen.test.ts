/**
 * Unit tests for updateAccountDeletionRequestsAction
 *
...
vi.mock(
  "@workspace/supabase-data/modules/account-management/infrastructure/repositories/account-deletion-requests-supabase.repository.codegen",
  () => ({
    AccountDeletionRequestsSupabaseRepository: vi.fn(),
  })
)
...
describe("updateAccountDeletionRequestsAction", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe("Authentication", () => {
    it("should throw Unauthorized when not authenticated", async () => {
      ...
      await expect(updateAccountDeletionRequestsAction({} as any)).rejects.toThrow("Unauthorized")
    })
    ...
    it("should log security.audit on unauthorized access", async () => {
      ...
      await updateAccountDeletionRequestsAction({} as any).catch(swallowExpectedError)
  ...
  describe("Input Validation", () => {
    it("should validate input with Zod schema", async () => {
      ...
      await expect(
        updateAccountDeletionRequestsAction({ invalid: "data" } as any)
      ).rejects.toThrow()
    })
    ...
    it("should accept valid input for update operation", async () => {
      const { getClaims } = await import("@workspace/supabase-auth/session/get-claims")
      const { checkActionRateLimit } = await import("@workspace/supabase-data/lib/auth/rate-limit")
      const { AccountDeletionRequestsSupabaseRepository } =
        await import("@workspace/supabase-data/modules/account-management/infrastructure/repositories/account-deletion-requests-supabase.repository.codegen")
      vi.mocked(getClaims).mockResolvedValue({ claims: { sub: "test-user-id" } } as any)
      vi.mocked(checkActionRateLimit).mockResolvedValue(undefined)
      ...
      vi.mocked(AccountDeletionRequestsSupabaseRepository).mockImplementation(
      ...
      // TODO: Replace with actual valid input for account_deletion_requests
      const validInput = {} as any

      await expect(updateAccountDeletionRequestsAction(validInput)).resolves.not.toThrow()
    })
  })
