/**
 * Unit tests for insertPsychologistPreferencesAction
 *
...
vi.mock(
  "@workspace/supabase-data/modules/preferences/infrastructure/repositories/psychologist-preferences-supabase.repository.codegen",
  () => ({
    PsychologistPreferencesSupabaseRepository: vi.fn(),
  })
)
...
describe("insertPsychologistPreferencesAction", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe("Authentication", () => {
    it("should throw Unauthorized when not authenticated", async () => {
      ...
      await expect(insertPsychologistPreferencesAction({} as any)).rejects.toThrow("Unauthorized")
    })
    ...
    it("should log security.audit on unauthorized access", async () => {
      ...
      await insertPsychologistPreferencesAction({} as any).catch(swallowExpectedError)
  ...
  describe("Input Validation", () => {
    it("should validate input with Zod schema", async () => {
      ...
      await expect(
        insertPsychologistPreferencesAction({ invalid: "data" } as any)
      ).rejects.toThrow()
    })
    ...
    it("should accept valid input for insert operation", async () => {
      const { getClaims } = await import("@workspace/supabase-auth/session/get-claims")
      const { checkActionRateLimit } = await import("@workspace/supabase-data/lib/auth/rate-limit")
      const { PsychologistPreferencesSupabaseRepository } =
        await import("@workspace/supabase-data/modules/preferences/infrastructure/repositories/psychologist-preferences-supabase.repository.codegen")
      vi.mocked(getClaims).mockResolvedValue({ claims: { sub: "test-user-id" } } as any)
      vi.mocked(checkActionRateLimit).mockResolvedValue(undefined)
      ...
      vi.mocked(PsychologistPreferencesSupabaseRepository).mockImplementation(
      ...
      // TODO: Replace with actual valid input for psychologist_preferences
      const validInput = {} as any

      await expect(insertPsychologistPreferencesAction(validInput)).resolves.not.toThrow()
    })
  })
