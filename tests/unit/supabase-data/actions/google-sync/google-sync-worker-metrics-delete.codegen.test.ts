/**
 * Unit tests for deleteGoogleSyncWorkerMetricsAction
 *
...
vi.mock(
  "@workspace/supabase-data/modules/google-sync/infrastructure/repositories/google-sync-worker-metrics-supabase.repository.codegen",
  () => ({
    GoogleSyncWorkerMetricsSupabaseRepository: vi.fn(),
  })
)
...
describe("deleteGoogleSyncWorkerMetricsAction", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe("Authentication", () => {
    it("should throw Unauthorized when not authenticated", async () => {
      ...
      await expect(deleteGoogleSyncWorkerMetricsAction({} as any)).rejects.toThrow("Unauthorized")
    })
    ...
    it("should log security.audit on unauthorized access", async () => {
      ...
      await deleteGoogleSyncWorkerMetricsAction({} as any).catch(swallowExpectedError)
  ...
  describe("Input Validation", () => {
    it("should validate input with Zod schema", async () => {
      ...
      await expect(
        deleteGoogleSyncWorkerMetricsAction({ invalid: "data" } as any)
      ).rejects.toThrow()
    })
    ...
    it("should accept valid input for delete operation", async () => {
      const { getClaims } = await import("@workspace/supabase-auth/session/get-claims")
      const { checkActionRateLimit } = await import("@workspace/supabase-data/lib/auth/rate-limit")
      const { GoogleSyncWorkerMetricsSupabaseRepository } =
        await import("@workspace/supabase-data/modules/google-sync/infrastructure/repositories/google-sync-worker-metrics-supabase.repository.codegen")
      vi.mocked(getClaims).mockResolvedValue({ claims: { sub: "test-user-id" } } as any)
      vi.mocked(checkActionRateLimit).mockResolvedValue(undefined)
      ...
      vi.mocked(GoogleSyncWorkerMetricsSupabaseRepository).mockImplementation(
      ...
      // TODO: Replace with actual valid input for google_sync_worker_metrics
      const validInput = {} as any

      await expect(deleteGoogleSyncWorkerMetricsAction(validInput)).resolves.not.toThrow()
    })
  })
