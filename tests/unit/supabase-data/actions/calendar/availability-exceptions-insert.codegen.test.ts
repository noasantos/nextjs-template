/**
 * Unit tests for insertAvailabilityExceptionsAction
 *
...
vi.mock(
  "@workspace/supabase-data/modules/calendar/infrastructure/repositories/availability-exceptions-supabase.repository.codegen",
  () => ({
    AvailabilityExceptionsSupabaseRepository: vi.fn(),
  })
)
...
describe("insertAvailabilityExceptionsAction", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe("Authentication", () => {
    it("should throw Unauthorized when not authenticated", async () => {
      ...
      await expect(insertAvailabilityExceptionsAction({} as any)).rejects.toThrow("Unauthorized")
    })
    ...
    it("should log security.audit on unauthorized access", async () => {
      ...
      await insertAvailabilityExceptionsAction({} as any).catch(swallowExpectedError)
  ...
  describe("Input Validation", () => {
    it("should validate input with Zod schema", async () => {
      ...
      await expect(insertAvailabilityExceptionsAction({ invalid: "data" } as any)).rejects.toThrow()
    })

    it("should accept valid input for insert operation", async () => {
      const { getClaims } = await import("@workspace/supabase-auth/session/get-claims")
      const { checkActionRateLimit } = await import("@workspace/supabase-data/lib/auth/rate-limit")
      const { AvailabilityExceptionsSupabaseRepository } =
        await import("@workspace/supabase-data/modules/calendar/infrastructure/repositories/availability-exceptions-supabase.repository.codegen")
      vi.mocked(getClaims).mockResolvedValue({ claims: { sub: "test-user-id" } } as any)
      vi.mocked(checkActionRateLimit).mockResolvedValue(undefined)
      ...
      vi.mocked(AvailabilityExceptionsSupabaseRepository).mockImplementation(
      ...
      // TODO: Replace with actual valid input for availability_exceptions
      const validInput = {} as any

      await expect(insertAvailabilityExceptionsAction(validInput)).resolves.not.toThrow()
    })
  })
