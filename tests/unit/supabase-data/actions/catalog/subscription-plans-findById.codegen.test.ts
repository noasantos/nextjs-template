/**
 * Unit tests for findByIdSubscriptionPlansAction
 *
...
vi.mock(
  "@workspace/supabase-data/modules/catalog/infrastructure/repositories/subscription-plans-supabase.repository.codegen",
  () => ({
    SubscriptionPlansSupabaseRepository: vi.fn(),
  })
)
...
describe("findByIdSubscriptionPlansAction", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe("Authentication", () => {
    it("should throw Unauthorized when not authenticated", async () => {
      ...
      await expect(findByIdSubscriptionPlansAction({} as any)).rejects.toThrow("Unauthorized")
    })
    ...
    it("should log security.audit on unauthorized access", async () => {
      ...
      await findByIdSubscriptionPlansAction({} as any).catch(swallowExpectedError)
  ...
  describe("Input Validation", () => {
    it("should validate input with Zod schema", async () => {
      ...
      await expect(findByIdSubscriptionPlansAction({ invalid: "data" } as any)).rejects.toThrow()
    })
  })
  ...
  describe("Logging", () => {
    it("should log on success", async () => {
      const { getClaims } = await import("@workspace/supabase-auth/session/get-claims")
      const { checkActionRateLimit } = await import("@workspace/supabase-data/lib/auth/rate-limit")
      const { SubscriptionPlansSupabaseRepository } =
        await import("@workspace/supabase-data/modules/catalog/infrastructure/repositories/subscription-plans-supabase.repository.codegen")
      const { logServerEvent } = await import("@workspace/logging/server")
      vi.mocked(getClaims).mockResolvedValue({ claims: { sub: "test-user-id" } } as any)
      vi.mocked(checkActionRateLimit).mockResolvedValue(undefined)
      ...
      vi.mocked(SubscriptionPlansSupabaseRepository).mockImplementation(function MockRepository() {
        return {
