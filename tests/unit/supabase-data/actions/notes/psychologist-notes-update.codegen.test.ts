/**
 * Unit tests for updatePsychologistNotesAction
 *
...
vi.mock(
  "@workspace/supabase-data/modules/notes/infrastructure/repositories/psychologist-notes-supabase.repository.codegen",
  () => ({
    PsychologistNotesSupabaseRepository: vi.fn(),
  })
)
...
describe("updatePsychologistNotesAction", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  describe("Authentication", () => {
    it("should throw Unauthorized when not authenticated", async () => {
      ...
      await expect(updatePsychologistNotesAction({} as any)).rejects.toThrow("Unauthorized")
    })
    ...
    it("should log security.audit on unauthorized access", async () => {
      ...
      await updatePsychologistNotesAction({} as any).catch(swallowExpectedError)
  ...
  describe("Input Validation", () => {
    it("should validate input with Zod schema", async () => {
      ...
      await expect(updatePsychologistNotesAction({ invalid: "data" } as any)).rejects.toThrow()
    })

    it("should accept valid input for update operation", async () => {
      const { getClaims } = await import("@workspace/supabase-auth/session/get-claims")
      const { checkActionRateLimit } = await import("@workspace/supabase-data/lib/auth/rate-limit")
      const { PsychologistNotesSupabaseRepository } =
        await import("@workspace/supabase-data/modules/notes/infrastructure/repositories/psychologist-notes-supabase.repository.codegen")
      vi.mocked(getClaims).mockResolvedValue({ claims: { sub: "test-user-id" } } as any)
      vi.mocked(checkActionRateLimit).mockResolvedValue(undefined)
      ...
      vi.mocked(PsychologistNotesSupabaseRepository).mockImplementation(function MockRepository() {
        return {
          findById: vi.fn(),
          list: vi.fn(),
          insert: vi.fn(),
      ...
      // TODO: Replace with actual valid input for psychologist_notes
      const validInput = {} as any

      await expect(updatePsychologistNotesAction(validInput)).resolves.not.toThrow()
    })
  })
