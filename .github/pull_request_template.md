...
- [ ] No forbidden patterns were introduced (`packages/ui` edits, manual migrations, app-local DB layers, server `getSession()`, barrel re-exports).
...
- [ ] `pnpm test:coverage`
- [ ] `pnpm test:db` if schema/RLS/auth-sensitive persistence changed
