...
Este directório é o pacote **shadcn/ui** do monorepo. Trata-o como **gerado e imutável** para código de produto.
...
Se precisas de UI partilhada **escrita** por humanos (composições, marca, blocos reutilizáveis entre apps), isso vai para **`@workspace/brand`** (`packages/brand`), **não** para aqui.
...
**Se estás a ler isto antes de implementar uma feature:** confirma que não estás a editar `packages/ui` para a “resolver”. Usa `packages/brand` ou componentes da app.
