  ...
  return withServerObservabilityContext(context, async () => {
    ...
    if (!protection.ok) {
      ...
      await logServerEvent({
        ...
        eventName: isMisconfigured ? "callback_abuse_protection_required" : "callback_rate_limited",
        httpStatus,
        metadata: {
    ...
    if (!parsed.success) {
      ...
      return NextResponse.redirect(new URL("/?auth=callback_missing_code", requestUrl.origin))
    }
    ...
    if (result.error) {
      ...
      return NextResponse.redirect(new URL("/?auth=callback_error", requestUrl.origin))
    }
