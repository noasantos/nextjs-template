/**
 * useClinicalSessionDetailsMutation — TanStack Query mutation wrapper for a Server Action.
 *
 ...
 *
 * @module @workspace/supabase-data/hooks/clinical-sessions/use-clinical-session-details-mutation.hook
...
// TODO: import { insertClinicalSessionDetailsAction } from "@workspace/supabase-data/actions/clinical-sessions/clinical-session-details-insert.codegen"
// TODO: Narrow the input type based on your action's requirements
type MutationInput = unknown

export function useClinicalSessionDetailsMutation(): UseMutationResult<
  unknown,
  Error,
  MutationInput,
  unknown
> {
  ...
  return useMutation({
    mutationFn: async (_input: MutationInput) => {
      ...
      // return insertClinicalSessionDetailsAction(_input as any)
      throw new Error("Wire Server Action in mutationFn")
    },
    onSettled: async () => {
      // TODO: Invalidate precise query keys
      await queryClient.invalidateQueries({
        queryKey: clinicalSessionsQueryKeys.clinicalSessionDetails(),
      })
    },
  })
}
