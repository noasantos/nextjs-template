/**
 * useCalendarHolidaysMutation — TanStack Query mutation wrapper for a Server Action.
 *
 ...
 *
 * @module @workspace/supabase-data/hooks/calendar/use-calendar-holidays-mutation.hook
...
// TODO: import { insertCalendarHolidaysAction } from "@workspace/supabase-data/actions/calendar/calendar-holidays-insert.codegen"
// TODO: Narrow the input type based on your action's requirements
type MutationInput = unknown

export function useCalendarHolidaysMutation(): UseMutationResult<
  unknown,
  Error,
  MutationInput,
  unknown
> {
  ...
  return useMutation({
    mutationFn: async (_input: MutationInput) => {
      ...
      // return insertCalendarHolidaysAction(_input as any)
      throw new Error("Wire Server Action in mutationFn")
    },
    onSettled: async () => {
      // TODO: Invalidate precise query keys
      await queryClient.invalidateQueries({ queryKey: calendarQueryKeys.calendarHolidays() })
    },
  })
}
