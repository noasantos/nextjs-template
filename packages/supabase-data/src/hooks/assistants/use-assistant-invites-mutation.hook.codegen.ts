/**
 * useAssistantInvitesMutation — TanStack Query mutation wrapper for a Server Action.
 *
 ...
 *
 * @module @workspace/supabase-data/hooks/assistants/use-assistant-invites-mutation.hook
...
// TODO: import { insertAssistantInvitesAction } from "@workspace/supabase-data/actions/assistants/assistant-invites-insert.codegen"
// TODO: Narrow the input type based on your action's requirements
type MutationInput = unknown

export function useAssistantInvitesMutation(): UseMutationResult<
  unknown,
  Error,
  MutationInput,
  unknown
> {
  ...
  return useMutation({
    mutationFn: async (_input: MutationInput) => {
      ...
      // return insertAssistantInvitesAction(_input as any)
      throw new Error("Wire Server Action in mutationFn")
    },
    onSettled: async () => {
      // TODO: Invalidate precise query keys
      await queryClient.invalidateQueries({ queryKey: assistantsQueryKeys.assistantInvites() })
    },
  })
}
