/**
 * useGoogleSyncLogsMutation — TanStack Query mutation wrapper for a Server Action.
 *
 ...
 *
 * @module @workspace/supabase-data/hooks/google-sync/use-google-sync-logs-mutation.hook
...
// TODO: import { insertGoogleSyncLogsAction } from "@workspace/supabase-data/actions/google-sync/google-sync-logs-insert.codegen"
// TODO: Narrow the input type based on your action's requirements
type MutationInput = unknown

export function useGoogleSyncLogsMutation(): UseMutationResult<
  unknown,
  Error,
  MutationInput,
  unknown
> {
  ...
  return useMutation({
    mutationFn: async (_input: MutationInput) => {
      ...
      // return insertGoogleSyncLogsAction(_input as any)
      throw new Error("Wire Server Action in mutationFn")
    },
    onSettled: async () => {
      // TODO: Invalidate precise query keys
      await queryClient.invalidateQueries({ queryKey: googleSyncQueryKeys.googleSyncLogs() })
    },
  })
}
