...
This document is the **contract for this template**: what is intentionally pinned and where it lives. When you fork the repo, treat versions here as the source of truth unless you are upgrading the whole stack.

**Operational rules** (imports, migrations, tests) stay in [repository-standards.md](../standards/repository-standards.md) and [AGENTS.md](../AGENTS.md).
...
| **Tailwind** | `^4.1.x` | `@tailwindcss/postcss`, `tailwindcss` in UI + apps |
| **Zod** | `^4.3.x` | Example app, data layer, forms, logging, UI |
| **Supabase JS** | `^2.100.x` | `@supabase/supabase-js` |
| **Supabase SSR** | `^0.9.x` | `@supabase/ssr` |
| **Supabase CLI** | `^2.83.x` | Root `devDependencies` |
| **Turborepo** | `^2.8.x` | Root |
| **Vitest** | `^3.2.x` | Root + packages + apps |
| **ESLint** | `^9.39.x` | Shared config + consumers |
...
| Layer | Choice | Notes |
|--------|--------|--------|
| **Framework** | **Next.js 16** (App Router) | `next` + `@next/env` in apps; the template `example` app uses **Webpack** in dev (`next dev --webpack`) — see `apps/example/package.json` → `dev`. |
| **React** | **19.x** | Server Components by default. |
| **Build** | `next build --webpack` | As configured in apps (align with dev bundler choices when changing). |
...
Setup: [docs/guides/supabase-setup.md](../guides/supabase-setup.md). Types: `pnpm supabase:types:local` / `supabase:types:linked`.
...
| **ESLint** `9.x` (flat) | `@workspace/eslint-config`: `base`, `next-js`, `next-app`, `react-internal`; **@next/eslint-plugin-next**, **typescript-eslint** `^8.x`, **eslint-plugin-react-hooks**, **eslint-plugin-turbo**. |
...
1. Bump **Next** and **`@next/env`** (and **`@next/eslint-plugin-next`** in eslint-config) together.  
2. Bump **React** / **React DOM** together everywhere they appear.  
3. Bump **Zod** across **all** workspace `package.json` files that declare it, then `pnpm install`, `pnpm typecheck`, tests.  
4. Bump **Tailwind** / **`@tailwindcss/postcss`** together in UI and apps.  
5. After **Supabase CLI** or DB changes, `pnpm supabase:types:local` ([DATABASE.md](../architecture/database.md)).  
6. **Turborepo** + **eslint-plugin-turbo**: keep versions loosely aligned when upgrading Turbo.
...
- [docs/guides/observability-architecture.md](../guides/observability-architecture.md) — logging contracts  
- [docs/guides/client-ui-data-sync.md](../guides/client-ui-data-sync.md) — mutations and cache  
- [docs/reference/command-reference.md](./command-reference.md) — npm/pnpm scripts  
- [docs/standards/golden-rules.md](../standards/golden-rules.md) — GR-006, GR-015, GR-017, GR-018, GR-019  
