...
> [`.cursor/rules/zod-v4-syntax.mdc`](../../../.cursor/rules/zod-v4-syntax.mdc).
> Regenerate: `node scripts/ci/sync-cursor-rules-to-docs.mjs`.

---

# Zod v4 Syntax Requirement

## Mandatory Patterns

### 1. Use `.inputSchema()` for next-safe-action v8
...
### 5. Common Number Validators

```typescript
// Positive only
const age = z.number().positive()
...
1. **Zod v4** has breaking changes from v3
2. **next-safe-action v8** uses `.inputSchema()` not `.schema()`
3. **Type inference** depends on correct syntax
4. **AI agents** learn from these patterns and replicate them

## Documentation

See [`docs/tools/zod-v4-patterns.md`](/docs/tools/zod-v4-patterns.md) for
complete reference.

## Examples

See [`apps/example/actions/example.ts`](/apps/example/actions/example.ts) for
working example.
